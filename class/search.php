...
class search extends config {
    ...
    public function search() {
        ...
        $stmt = $connection->prepare("SELECT * FROM `tbl_users` WHERE `id` LIKE '%$search%' OR `firstname` LIKE '%$search%' OR `lastname` LIKE '%$search%' OR `age` LIKE '%$search%' OR `username` LIKE '%$search%'");
        $stmt->execute();
        $data = $stmt->fetchAll();
        $count = $stmt->rowCount();
